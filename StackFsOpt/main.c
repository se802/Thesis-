                              ...
                              const char *name)
{
    ...
    if (res == 0) {
        ...
        if (!inode)
            ...
            e.ino = inode->lo_ino;
            //printf("replying to kernel1");
            fuse_reply_entry(req, &e);
        }
    } else {
        if (fullPath)
            free(fullPath);
        //printf("replying to kernel2");
...
int main(int argc, char **argv)
{
    ...
    if (res != -1) {
        struct fuse_session *se= fuse_session_new(&args, &ll_ops, sizeof(ll_ops), lo);
        fuse_set_signal_handlers(se);
        int x = fuse_session_mount(se, opts.mountpoint);
        fuse_daemonize(true);
